...
#loading tidyverse, then reading the same Kings dataset using 5 different function combinations to compare how base R and tidyverse functions handle the data differently.
library(tidyverse)
Kings <- read_csv2("data/Kings_fil.csv")

kings1 <- read.csv("data/Kings_fil.csv")

kings2 <- read_csv("data/Kings_fil.csv")

kings3 <- read.csv2("data/Kings_fil.csv")

kings4 <- read_csv2("data/Kings_fil.csv")
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
read_csv() is the tidyverse function (from the readr package).
i have been using read_csv2() which is for semicolon-delimited files

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.

tells me that the kings object is:

- A tibble (tbl_df, tbl) - the tidyverse's enhanced data frame
- till a data.frame at its core
- as a spec_tbl_df class (spec = specification - tracks column types)
...
This is shown in the glimpse(kings) output: df [6 × 7]

4. Show the dataset so that we can see how R interprets each column
To view the data i use "Class" and i use "glimpse"... class in this context shows the tyoe of obejct the data is, and it tells me that R recognizes Kings as a data frame (a table-like structure)
Glimpse shows a compact, transposed view of my entire dataset structure... it furthermore shows how many rows and columns i have
and each of the columns names and how R interprets each column (data type: <chr> for text, <dbl> for numbers, etc.) and a preview of the actual values
See solution here below↓

```{r eval = TRUE} 
# COMPLETE THE BLANKS BELOW WITH YOUR CODE, then turn the 'eval' flag in this chunk to TRUE.
# inspecting the kings object by checking its class type, displaying a compact overview of its structure, and showing the first few rows of data.
class(Kings)
...
```{r eval = TRUE}
# YOUR CODE
#creating a duration column by subtracting start reign from end reign, then displaying the dataset to see the results.
Kings <- Kings %>%
...
```{r eval = TRUE}
# YOUR CODE
#creating a duration column (in years) by subtracting start reign from end reign, then calculating the average reign duration.
Kings <- Kings %>%
  mutate(duration = as.numeric(Reg_slut) - as.numeric(Reg_start))
...
```{r eval = TRUE}
# YOUR CODE
#calculating the average reign duration, then counting how many kings reigned longer than average.
Kings <- Kings %>%
  mutate(duration = as.numeric(Reg_slut) - as.numeric(Reg_start))
...
Kings %>%
  filter(duration > mean(Kings$duration, na.rm = TRUE)) %>%
  count()
...
```{r eval = TRUE}
# YOUR CODE
#finding the 3 longest-ruling monarchs and converting their reign duration from years to days.
...
#Answer is
#Christian 4.	60	21960		
#Margrethe 2.	52	19032		
#Christian 9.	43	15738	 
...
#creating a line and point plot showing how the duration of reign changed over Danish history, with the start year on the x-axis and reign duration on the y-axis.

Kings %>%
  ggplot(aes(x = Reg_start, y = duration)) +
  geom_point() +
  geom_line() +
